...
有关启用平台事件的详细信息，请参见 link:../config/concept_active_iq_platform_events.html["启用 Active IQ 门户事件"]。
有关上传规则文件的详细信息，请参见 link:../events/task_upload_new_active_iq_rules_file.html["上传新的 Active IQ 规则文件"]。

NetApp Active IQ 是一种基于云的服务，可提供预测分析和主动支持，以优化整个 NetApp 混合云的存储系统运行。有关详细信息，请参见 https://docs.netapp.com/us-en/active-iq/index.html["NetApp Active IQ"^]。
